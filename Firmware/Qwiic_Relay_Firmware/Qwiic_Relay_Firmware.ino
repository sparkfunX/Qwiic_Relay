...
#define I2C_ADDRESS_DEFAULT 0x18
...
void setup() {
	...
	if(qwiicRelayAddress == 0xFF){
		...
		qwiicRelayAddress = I2C_ADDRESS_DEFAULT; //default
	}
	
	pinMode(RELAY_PIN, OUTPUT);
	digitalWrite(RELAY_PIN, LOW); //Make sure relay is off at power on

	TinyWire.begin(qwiicRelayAddress);	
	TinyWire.onReceive(receiveEvent); // Attach interrupts
